"""The finished job architecture as one read model.

instructions.txt, final requirement: "Ability to browse full hierarchy of job family, job
category, anchor role, and role detail including skills and tasks", with headcount analytics
and the 3rd-party taxonomy match.

Everything downstream of clustering keys off `profile_key`, so this is a join rather than a
recomputation: role document + JE result + required skills + tasks + external taxonomy match,
rolled up through Family > Category > Anchor role with headcount at every tier.

Lifted out of the overview route so the browser and the HTML report read the same thing. Two
builders over one hierarchy would eventually disagree, and the report is a client deliverable —
it is the worse of the two places to find out.
"""
...
def build(state: ProjectState) -> dict | None:
    """The whole architecture, or None when the hierarchy has not been named yet."""
    c = state.clustering
